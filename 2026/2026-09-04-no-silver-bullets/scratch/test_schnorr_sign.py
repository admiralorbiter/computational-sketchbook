...
print(f"Collected {len(relations)} relations with sign.")
if len(relations) >= n + 1:
    ...
    for dep in deps:
        ...
        if tot_sign != 0:
            continue # sum of signs must be even
        ...
        if 1 < g1 < N:
            print(f"SUCCESS! Factor found: {g1}, {N // g1}")
            break
        if 1 < g2 < N:
            print(f"SUCCESS! Factor found: {g2}, {N // g2}")
            break
