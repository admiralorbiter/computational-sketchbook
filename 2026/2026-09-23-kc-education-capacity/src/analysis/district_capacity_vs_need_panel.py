...
def run_district_capacity_vs_need_analysis():
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"   - **Finding:** KCPS aggressively expanded certified teacher staffing relative to enrollment (+21.7% teacher density growth), outstripping its special education accommodation rate and reaching 84.2 teacher FTE per 1,000 students (11.88 students per teacher FTE).\n\n")
        ...
        f.write(f"   - **Finding:** While regional peers added +6 to +11 teachers per 1,000 students (e.g. Grandview +10.7, Blue Valley +10.3, SMSD +9.2, NKC +6.8), KCKPS experienced acute capacity stagnation and erosion, leaving it severely strained under massive EL and SPED loads.\n\n")
        ...
        f.write(f"   - **YES. Suburban districts expanded teacher density while accommodation rates rose (with Shawnee Mission specifically documenting the planning-time schedule transition):**\n")
        ...
        f.write(f"     - **Blue Valley:** Added **+{bv_g['delta_tch_1000']:.1f} teachers per 1,000 (+{bv_g['pct_change_tch_1000']:.1f}%)**, reducing PTR from {bv_g['ptr_2015']:.2f}:1 to {bv_g['ptr_2023']:.2f}:1 ({bv_g['delta_ptr']:+.2f}), while Section 504 rose from {bv_g['sec504_pct_2015']:.2f}% to {bv_g['sec504_pct_2023']:.2f}%.\n")
        f.write(f"     - **North Kansas City:** Added **+{nkc_g['delta_tch_1000']:.1f} teachers per 1,000 (+{nkc_g['pct_change_tch_1000']:.1f}%)**, driving PTR from {nkc_g['ptr_2015']:.2f}:1 to {nkc_g['ptr_2023']:.2f}:1 ({nkc_g['delta_ptr']:+.2f}), while accommodations rose from {nkc_g['acc_pct_2015']:.2f}% to {nkc_g['acc_pct_2023']:.2f}% (+{nkc_g['delta_acc_pct']:.2f} pp).\n")
        f.write(f"     - **Lee's Summit:** Added **+{ls_g['delta_tch_1000']:.1f} teachers per 1,000 (+{ls_g['pct_change_tch_1000']:.1f}%)**, driving PTR down from {ls_g['ptr_2015']:.2f}:1 to {ls_g['ptr_2023']:.2f}:1 ({ls_g['delta_ptr']:+.2f}), while accommodations rose from {ls_g['acc_pct_2015']:.2f}% to {ls_g['acc_pct_2023']:.2f}% (+{ls_g['delta_acc_pct']:.2f} pp).\n\n")
        
        # Falling PTR vs Rising SPED/504
        f.write(f"5. **Which districts saw falling students-per-FTE but rising SPED/504 burden?**\n")
        f.write(f"   - **Almost the entire region!** Across the 56 eligible longitudinal districts (LEAs with baseline enrollment ≥ 400 present across both endpoints), **48 districts (85.7% of the eligible sample)** saw students-per-teacher FTE fall (improved headline staffing) while their student accommodation share increased.\n")
