...
- **Phase 4A Launch (Course Section Microdata Requests):**
  ...
  - **District Level Requests (Track B):** Packaged in `requests/districts/` (`DISTRICT_RECORDS_REQUEST_FRAMEWORK.md`, `kansas_kora_district_requests.md`, `missouri_sunshine_district_requests.md`, and `district_request_ledger.csv`) covering 9 target LEAs requesting existing SIS master schedule section exports without student PII.
  - **Photographic Validation Sample (Track C):** Fully designed in `research/elementary_yearbook_sampling_protocol.md` establishing a 12-school stratified purposive matrix with pre-specified replacements, a 2-school feasibility pilot, and absolute agreement IRR metrics, supported by `data/raw/yearbooks/yearbook_homeroom_ledger.csv`.
